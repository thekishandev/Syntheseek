import { cerebrasClient } from "@/utils/clients";
...
Generate 3 related follow-up questions based on the original question and the search results above.`,
  ...
  const content = (similarQuestions as any)?.choices?.[0]?.message?.content
  let response = content || '{"questions": []}'
...
export async function POST(request: Request) {
  ...
  // Create context from sources with reduced length to avoid context length errors
